...
class FCDis(nn.Module):
    def __init__(self, num_puzzle=9, size_puzzle=75):
        super(FCDis, self).__init__()
    ...
    def shuffle(self, x, perm):
        ...
        if perm == 'random':
            ...
            assert perm.size() == (N, self.num_puzzle), f"perm shape error, got {perm.size()}"
            noise = perm
            ids_shuffles = torch.argsort(noise, dim=1)
            ids_restores = torch.argsort(ids_shuffles, dim=1)
    ...
    def forward(self, x, perm='random'):
        x, target = self.shuffle(x, perm)
        x = x[:, :, :-1, :-1]
...
if __name__ == '__main__':
    noise = torch.rand(2, 9)
    model = FCDis()
    output, target = model(torch.rand(2, 3, 225, 225), noise)
