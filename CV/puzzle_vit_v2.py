...
class PuzzleViT(nn.Module):
    def __init__(self, num_puzzle=9, size_puzzle=75, threshold=0.8):
        ...
        self.puzzle_head_fc1 = nn.Linear(768, 1000)
        self.puzzle_head_fc2 = nn.Linear(1000, 1000)
        self.puzzle_head_fc3 = nn.Linear(1000, self.num_puzzle * 2)
        self.map_values = []
        self.map_coord = None
        self.min_dist = 0
    ...
    def forward(self, x):
        ...
        return x, target, loss_var, x_cls
...
if __name__ == '__main__':
    model = PuzzleViT()
    output, target, loss_var, x_cls = model(torch.rand(2, 3, 225, 225))
